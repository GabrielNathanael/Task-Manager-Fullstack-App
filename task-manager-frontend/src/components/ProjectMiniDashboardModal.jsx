  ...
  return (
    <div className="bg-white rounded-lg shadow-md mt-6 overflow-hidden">
      ...
      <div className="px-4 py-2">
        <div className="relative flex-grow mx-auto max-w-md">
          <input
            type="text"
            placeholder="Search..."
            value={search}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className={`${row.original.isOptimistic ? 'opacity-50 animate-pulse' : ''}`}
                >
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} className="px-6 py-4 text-sm align-top text-center">
...
const ProjectMiniDashboardModal = ({
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        const res = await axios.get(`http://localhost:8000/api/tasks?project_id=${project.id}`, {
  ...
  const handleAddTask = async (form) => {
    ...
    const optimisticTask = { ...form, id: optimisticId, isOptimistic: true, project };
    const updated = [optimisticTask, ...tasks];
    setTasks(updated);
    updateProjectCounts(updated);
    setShowAddTaskModal(false);
    try {
      const res = await axios.post('http://localhost:8000/api/tasks', { ...form, project_id: project.id }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('sanctum_token')}` },
      });
      const final = updated.map((t) => (t.id === optimisticId ? { ...res.data.task, project } : t));
      setTasks(final);
  ...
  const handleUpdateTask = async (taskId, data) => {
    ...
    setEditingTask(null);
    try {
      const res = await axios.put(`http://localhost:8000/api/tasks/${taskId}`, data, {
        headers: { Authorization: `Bearer ${localStorage.getItem('sanctum_token')}` },
      });
      const final = tasks.map((t) => (t.id === taskId ? { ...res.data.task, project } : t));
      setTasks(final);
      updateProjectCounts(final);
    } catch {
      setTasks(original);
    ...
    <div className="fixed inset-0 bg-white/30 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl min-h-[500px]">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold text-gray-800">Project: {project.name}</h3>
