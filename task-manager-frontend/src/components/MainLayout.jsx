...
import { FolderIcon, NotificationIcon, PlusIcon, LogoutIcon, DashboardIcon, SearchIcon } from '../assets/icons/Icons.jsx';
    ...
    return (
        <aside className="w-64 bg-white shadow-md flex-shrink-0 border-r border-gray-200 p-4">
            ...
            <nav className="space-y-2">
                <Link
                    to="/dashboard"
                    className={`flex items-center space-x-3 p-2 rounded-lg font-semibold ${location.pathname.startsWith('/dashboard') ? 'bg-blue-100 text-blue-800' : 'text-gray-700 hover:bg-gray-100'}`}
                >
                ...
                <Link
                    to="/projects"
                    className={`flex items-center space-x-3 p-2 rounded-lg font-semibold ${location.pathname.startsWith('/projects') ? 'bg-blue-100 text-blue-800' : 'text-gray-700 hover:bg-gray-100'}`}
                >
...
const MainLayout = ({ children }) => {
    // 1. Ambil SEMUA nilai yang dibutuhkan dari context di sini, SATU KALI SAJA.
    const { isAuthenticated, handleLogout, loadingAuth } = useAuth();
    const [userData, setUserData] = useState(null);
    const navigate = useNavigate();

    useEffect(() => {
        // 2. Gunakan variabel `loadingAuth` yang sudah ada, jangan panggil useAuth() lagi.
        ...
        // Hanya jalankan fetchUserData jika proses loading auth awal sudah selesai.
        if(!loadingAuth) {
            fetchUserData();
        }
    }, [isAuthenticated, loadingAuth, navigate, handleLogout]);

    // Jika proses auth awal masih berjalan, jangan render apa-apa
    // Ini mencegah "kedipan" singkat ke halaman login saat refresh
    if (loadingAuth) {
        return null;
    }
